...
df = pd.read_csv("backend/ai/curator/clusterer.py", encoding = "ISO-8859-1")
with open('backend/ai/curator/20k.txt') as f:
    words = f.readlines()
corpus = []
for word in words:
...
for index, row in df.iterrows():
    titleCorpus.append([row["Heading"]])
titleTokenizer.fit_on_texts(titleCorpus)

titleOHEs = titleTokenizer.texts
